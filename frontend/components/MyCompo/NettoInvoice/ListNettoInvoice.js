...
import { useGetAllNettoInvoicesQuery ,useDeleteNettoInvoiceMutation } from "@/redux/slices/nettoInvoiceApiSlice"; // Assuming you have a similar API slice for netto invoices
import { useRouter } from 'next/router';
import SearchBar from "@/components/MyCompo/NettoInvoice/SearchBar"; // Path adjusted for NettoInvoice
import Pagination from "@/components/MyCompo/NettoInvoice/Pagination"; // Path adjusted for NettoInvoice
...
const ListNettoInvoice = ( ) => {
    const router = useRouter();
    const { t } = useTranslation('common');
    const { data: nettoInvoices, isLoading, error } = useGetAllNettoInvoicesQuery();
    const [searchTerm, setSearchTerm] = useState('');
    const [page, setPage] = useState(0);
    const [rowsPerPage, setRowsPerPage] = useState(10);
    const [searchTermInput, setSearchTermInput] = useState('');
    const [itemToDelete, setItemToDelete] = useState(null);
    const [deleteItem] = useDeleteNettoInvoiceMutation();
    ...
    useEffect(() => {
        if(nettoInvoices?.data) {
            const filteredInvoices = searchTerm
                ? nettoInvoices?.data.filter((agreement) => {
    ...
    if (!nettoInvoices?.data) {
        return <div>No Netto invoices found.</div>;
    }
    // const filteredInvoices = searchTerm
    //     ? nettoInvoices?.data.filter((agreement) => {
    //         const searchTermLower = searchTerm.toLowerCase().trim();
                ...
                >
                    ...
                    <TableBody>
                        {(rowsPerPage > 0
                            ...
                            <TableRow key={invoice._id} sx={{backgroundColor :invoice?.invoiceDetails?.status ==='canceled' ?'#e8adad':'' }}>
                                <TableCell >
                                    {invoice.invoiceDetails?.invoiceNumber}
                                </TableCell>
                                <TableCell>
                                    {/* Assume you have a function to concatenate product details */}
                                    {invoice.carDetails ? 
...
export default ListNettoInvoice;
