...
import { useGetAllMwstInvoicesQuery ,useDeleteMwstInvoiceMutation } from "@/redux/slices/mwstInvoiceApiSlice";
import EditIcon from "@mui/icons-material/Edit";
import ClearIcon from "@mui/icons-material/Clear";
import {useRouter} from 'next/router';
import SearchBar from "@/components/MyCompo/MwstInvoice/SearchBar";
import Pagination from "@/components/MyCompo/MwstInvoice/Pagination";
...
const ListMwstInvoice = ( ) => {
    const router = useRouter();
    const { t } = useTranslation('common');
    const { data: mwstInvoices, isLoading, error } = useGetAllMwstInvoicesQuery();
    const [searchTerm, setSearchTerm] = useState('');
    const [page, setPage] = useState(0);
    const [rowsPerPage, setRowsPerPage] = useState(10);
    const [searchTermInput, setSearchTermInput] = useState('');
    const [itemToDelete, setItemToDelete] = useState(null);
    const [deleteItem] = useDeleteMwstInvoiceMutation();
    ...
    // Assuming you have some logic to filter the invoices based on the searchTerm and calculate emptyRows
                    ...
                    <TableBody>
                        {(rowsPerPage > 0
                            ...
                            <TableRow key={invoice._id} sx={{backgroundColor :invoice?.invoiceDetails?.status ==='canceled' ?'#e8adad':'' }}>
                                ...
                                <TableCell align="center">
                                    <ActionButtons
                                        pdfUrl={   process.env.NEXT_PUBLIC_BACKEND_URL.replace('/api/v1', '')+invoice.invoiceDetails.invoiceLink.replace(/\\/g, '/')  }
...
export default ListMwstInvoice;
